...
my_map_t *stuck_box2(my_map_t *my_map, my_player_t **my_player)
{
    if (my_map->map[(*my_player)->x][(*my_player)->y - 3] == '#' && my_map->map[(*my_player)->x - 1]\
[(*my_player)->y - 2] == '#') {
        my_map->lose += 1;
    }
    else if(my_map->map[(*my_player)->x][(*my_player)->y - 3] == '#' && my_map->map[(*my_player)->x \
+ 1][(*my_player)->y - 2] == '#') {
        my_map->lose += 1;
    }
    return (my_map);
...
my_map_t *move_left(my_map_t *my_map, my_player_t **my_player)
{
    if (my_map->map[(*my_player)->x][(*my_player)->y - 1] == ' ') {
        ...
        if (my_map->map[(*my_player)->x][(*my_player)->y - 2] == ' ') {
            ...
            my_map->map[(*my_player)->x][(*my_player)->y - 1] = 'P';
            my_map->map[(*my_player)->x][(*my_player)->y] = ' ';
            my_map = stuck_box2(my_map, my_player);
